...
- Five applied database migrations (`drizzle/0000` through `0004`), replayed idempotently against a provisioned Neon Postgres database (`osblog-db`, Singapore region); `0004` retains published-slug ownership and direct redirect targets. The admin account is bootstrapped and three bilingual introduction posts are seeded.
- Permanent published-slug history: additive backfill, advisory-locked database guard, one-hop HTML/API `308` redirects, hidden-target `404` behavior, current-only SEO/sitemap output, and an editor warning for published slug changes.
...
- Browser publishing, moderation, responsive-docs and automated accessibility checks pass in the compiled E2E suite; a separate manual visual review is evidence in the work record rather than an enforced CI service. Live Vercel current-route/unknown-slug smoke passes on both aliases; a positive live historical alias awaits a real intentional rename because no old-slug fixture exists.
...
- A positive live historical-alias smoke fixture is still absent; local SQL/browser coverage proves the one-hop redirect behavior, and production migration/deployment/current-route safety are verified.
...
- Neon disposable-branch backup/restore and Vercel deployment-alias rollback rehearsals pass; native dump/restore tools, provider contention load testing and the live historical-alias fixture remain open — see [docs/backups-and-rollback.md](docs/backups-and-rollback.md).
