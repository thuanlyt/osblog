...
- `drizzle/` owns the five SQL migrations now applied to production, including `0004_post_slug_history.sql`, verified locally under UA-0073/UA-0077 and rolled out under UA-0080. `src/server/provision.ts` owns migration execution, slug-history preflight, admin bootstrap, and the optional content seed.
...
Migration `0004_post_slug_history.sql` is an additive registry now applied to production. It locks post/audit writes while backfilling current published slugs and valid `before_summary.slug` values whose audited status was published. Candidates join to an existing post by ID, are deduplicated by slug/owner and retain the earliest available timestamp. Missing targets and malformed audit slugs are excluded. A slug associated with multiple posts, or with a different current owner (even a draft), aborts the entire migration before any changes commit. The rollout preflight returned zero ownership conflicts; an operator must reconcile any future ambiguity and the migration never chooses a winner.

Before any future schema rollout, call the read-only `preflightSlugHistory(db)` helper with the operator's database handle. It returns counts only: malformed published audit candidates, missing targets, multiple historical owners, and conflicts with current owners. It can run on the pre-0004 schema. The UA-0080 rollout reviewed skipped evidence, verified a disposable Neon branch restore and recorded zero ownership conflicts before applying 0004. Local PGlite tests exercise competing application calls but serialize database sessions; they do not measure provider lock contention. Native dump/restore and provider contention load testing remain open.
...
- **Positive live historical-alias evidence remains open.** Registry, one-hop redirects and editor feedback are deployed; local SQL/browser tests prove the redirect path and live current/unknown safety smoke passes, but production has no existing old-slug alias to test without an intentional content rename.
