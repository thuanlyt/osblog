...
- `drizzle/` sở hữu năm migration SQL đã áp dụng vào production, gồm `0004_post_slug_history.sql`, được kiểm tra cục bộ dưới UA-0073/UA-0077 và rollout dưới UA-0080; [`src/server/provision.ts`](https://github.com/thuanlyt/osblog/blob/main/src/server/provision.ts) sở hữu việc chạy migration, preflight slug, bootstrap admin, và seed nội dung tùy chọn.
...
UA-0073/UA-0077 bổ sung và hoàn thiện migration `0004_post_slug_history.sql`, bảng `post_slug_history`, bộ phân giải URL cũ và cảnh báo trong editor. UA-0080 đã tạo checkpoint branch Neon, chạy preflight, áp dụng migration và deploy code; không ghi secret hay dữ liệu private vào evidence.
